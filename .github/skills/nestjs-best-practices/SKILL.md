...
description: "**ARCHITECTURE SKILL** — Mastering NestJS architecture patterns, dependency injection, error handling, security best practices, and distributed systems design. USE FOR: implementing new NestJS features (modules, controllers, services); reviewing code for architecture and security violations; refactoring legacy NestJS codebases; designing authentication layers, database abstractions, repository patterns, or event-driven architectures; optimizing performance and microservices patterns. DO NOT USE FOR: general TypeScript syntax; debugging runtime errors; infrastructure, DevOps, CI/CD; frontend code; configuration tasks; import/export questions; non-NestJS frameworks."
license: MIT
metadata:
  author: Kadajett
  version: "1.3.0"
...
- Writing new NestJS features (modules, controllers, services, providers)
- Implementing domain logic or business features in services
- Designing authentication, authorization, or permission layers
- Creating database abstractions (repository patterns, entities, migrations)
- Implementing event-driven or request/response patterns
- Setting up error handling, exception filters, or guards
- Refactoring existing NestJS code for architecture or security improvements
- Reviewing pull requests for architecture and design violations
- Optimizing database queries or implementing caching strategies

## When NOT to Apply

**Skip this skill for:**

- General TypeScript syntax or type system questions
- Debugging runtime errors or stack traces (use error logs instead)
- Infrastructure, DevOps, CI/CD, or deployment configuration
- Frontend code (Next.js, React, Vue, etc.)
- Configuration-only tasks (environment variables, .env files)
- Simple import/export or module setup questions
- Package manager or dependency upgrade issues
- IDE/editor configuration or VS Code settings
- Third-party library integration without architectural impact
- Non-NestJS projects or different frameworks (Express, Fastify, etc.)
...
For the complete guide with all rules expanded: `copilot.instructions.md`
